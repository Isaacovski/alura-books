# 📚 Projeto Alura Books

![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-61DAFB?logo=react&logoColor=black)
![JSON](https://img.shields.io/badge/JSON-000000?logo=json&logoColor=white)

---

## 💻 Sobre o Projeto

Este projeto de livros foi criado durante um curso **extraordinário da Alura**. Ele me permitiu explorar tanto o **back end** quanto o **front end**, aprendendo do zero como APIs funcionam e como organizar um projeto em React.

---

## ⚙️ Back End

O back end foi construído **100% em JavaScript puro**, com rotas, controladores e toda a lógica da API feita manualmente.  

A API controla os livros e oferece as seguintes funcionalidades:  

- Listar livros (**GET**)  
- Adicionar livros (**POST**)  
- Modificar livros (**PUT**)  
- Apagar livros (**DELETE**)  

Os dados eram armazenados em um **arquivo JSON**, deixando o site leve e rápido.  

> Foi nesse projeto que usei o **Postman** pela primeira vez e criei minha primeira API. Infelizmente, os arquivos do back end foram perdidos 😅  

---

## 🖥️ Front End

O front end foi desenvolvido em **React** e conta com:  

- Sistema de pesquisa  
- Filtro por categorias  
- Carrinho de compras  
- Organização de componentes e estado, aprimorando minhas habilidades em React  

---

## 🚀 Tecnologias Utilizadas

- **JavaScript**  
- **React**  
- **JSON (como banco de dados)**  

---

<img width="1349" height="593" alt="Capturar" src="https://github.com/user-attachments/assets/0dfe745f-0bcf-48d0-a872-a9f8129d2eef" />

